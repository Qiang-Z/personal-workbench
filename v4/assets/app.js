...
function v5RefreshBadges(){
  ...
  let inc=0,exp=0;
  (data.finances||[]).forEach(f=>{if(f.date&&f.date.slice(0,7)===mNow){if(f.type==='income')inc+=+f.amount||0;else exp+=+f.amount||0;}});
  const bal=inc-exp;
...
function totalAssets(){let v=0;(data.funds||[]).forEach(f=>v+=fundValue(f));(data.finances||[]).forEach(x=>{v+=(x.type==='income'?1:-1)*(+x.amount||0);});return v;}
...
function renderOverview(){
  ...
  // 本月收支
  const mNow=new Date().toISOString().slice(0,7);
  let mInc=0,mExp=0;(data.finances||[]).forEach(f=>{if(f.date&&f.date.slice(0,7)===mNow){if(f.type==='income')mInc+=+f.amount||0;else mExp+=+f.amount||0;}});
...
function renderReview(){
  ...
  const fin=(data.finances||[]).filter(f=>!f.gen&&inR(f.date,mon,sun));
...
function expandFinanceRecur(){
  ...
  (data.finances||[]).forEach(t=>{
    ...
    while(true){
      ...
      data.finances.push({id:uid(),date:ds,type:t.type,category:t.category,amount:t.amount,note:(t.note||'')+' ·🔁',recur:t.recur,gen:true,tplId:t.id});
      n++;
    }
  });
}
...
function openFinanceForm(id,ds){
  ...
  document.getElementById('fn_type').value=f?f.type:'income';
